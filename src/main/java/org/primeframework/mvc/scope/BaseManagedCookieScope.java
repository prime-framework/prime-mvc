...
public abstract class BaseManagedCookieScope<T extends Annotation> extends AbstractCookieScope<T> {
  ...
  protected Cookie processCookie(Cookie cookie, String fieldName, Class<?> type, T scope) {
    ...
    try {
      ...
      if (compress || encrypt) {
        cookie.value = CookieTools.fromCookie(cookieValue, true, true, encryptor, oldFunction, newFunction);
      } else {
        try {
          cookie.value = CookieTools.fromCookie(cookieValue, true, false, encryptor, oldFunction, newFunction);
        } catch (Throwable t) {
