 ...
 *     <a
 *     href="https://commons.apache.org/proper/commons-text/javadocs/api-release/org/apache/commons/text/StringTokenizer.html">
...
public class StrTokenizer implements ListIterator<String>, Cloneable {
  ...
  /**
   * Constructs a tokenizer splitting on the specified delimiter character and handling quotes using the specified quote
   * character.
   *
   * @param input the string which is to be parsed
   * @param delim the field delimiter character
  ...
  /**
   * Gets a new tokenizer instance which parses Comma Separated Value strings initializing it with the given input.  The
   * default for CSV processing will be trim whitespace from both ends (which can be overridden with the setTrimmer
   * method).
   * <p>
   ...
   * <p>
   * These characters are ignored when parsing the String, unless they are within a quoted region. The default value is
   ...
   * <p>
   * These characters are trimmed off on each side of the delimiter until the token or quote is found. The default value
   * is not to trim anything.
   *
   ...
   * <p>
   * This method exists to allow subclasses to add code before or after the tokenization. For example, a subclass could
   * alter the character array, offset or count to be parsed, or call the tokenizer multiple times on multiple strings.
   * It is also be possible to filter the results.
   * <p>
   * {@code StrTokenizer} will always pass a zero offset and a count equal to the length of the array to this method,
   * however a subclass may pass other values, or even an entirely different array.
   *
   ...
   * @param quoteStart the start position of the matched quote, 0 if no quoting
   * @param quoteLen   the length of the matched quote, 0 if no quoting
   * @return the starting position of the next field (the character immediately after the delimiter, or if end of string
   *     found, then the length of string
   */
