...
<input type="radio"[#if options[key].selected] checked="checked"[/#if] value="${key}"[@macros.append_attributes attributes=attributes excludes=['id']/]/><span class="radio-text">${options[key].text}</span>
</div>
[/#list]
</div>
</div>
